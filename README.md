...
不只是替换背景图片：主题预设会同步改变侧栏选中态、按钮、功能卡片、消息气泡、输入框、弹窗、链接、滚动条、圆角、边框和阴影，让整套界面保持统一的视觉语言。
...
无需手动编辑配置文件，可直接选择图片，并调整亮度、饱和度、模糊、焦点、区域颜色、透明度和文字配色。
...
- 内置“暖霞玻璃、午夜霓虹、樱花梦境、海盐微风”四套完整主题预设
- 统一定制导航、按钮、功能卡片、用户/豆包气泡、弹窗、输入焦点和滚动条
- 支持柔和玻璃、清晰描边、沉浸实体三种组件风格
- 自定义主题强调色、强调文字、卡片和气泡颜色、圆角与阴影
- 智能分析背景亮度，分别为侧栏、主区域和输入框选择深色或浅色文字
- 支持固定深色、固定浅色，以及自定义主要文字和次要文字颜色
...
2. 双击 `皮肤设置.cmd`。
3. 在“背景与效果”“颜色与透明度”“文字与对比度”“组件主题”四个页面中调整参数，点击“保存并应用”。
...
- `themePreset`：当前主题预设
- `componentStyle`：组件风格，可选 `soft`、`outline`、`solid`
- `accentColor` / `accentTextColor`：主题强调色及其上方文字颜色
- `cardColor`：推荐内容、工具和弹窗卡片颜色
- `userBubbleColor` / `assistantBubbleColor`：双方消息气泡颜色
- `cornerRadius`：卡片、按钮和输入框圆角
- `shadowStrength`：组件立体阴影强度
- `textColorMode`：文字模式，可选 `auto`、`dark`、`light`、`custom`
- `textColor`：自定义模式下的主要文字颜色
- `mutedTextColor`：自定义模式下的次要文字颜色
- `blurPixels`：玻璃模糊半径

智能模式会对背景图片进行低分辨率本地采样，并结合亮度、遮罩和三个玻璃区域的颜色与透明度计算文字对比度。分析只在本机完成，不会上传图片。
